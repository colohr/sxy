...
class ScalarRouter{
	...
	route(name,value){
		if('routes' in this) {
			if(name in this.routes) value = this.routes[name](value)
			if(!fxy.is.nothing(value) && 'out' in this.routes) return this.routes.out(value,fxy)
		}
		return value
	}
	//serialize
	get(value){
		return this.route('get', get_value(this.type,{value}))
	}
	//parseLiteral
	literal(ast){
		return this.route('literal', get_value(this.type,ast))
	}
	//parseValue
	value(value) {
		return this.route('value', get_value(this.type,{value}))
	}
	get Type(){
		return {
...
function get_value(type,{value}){
	...
	if(type in fxy.id && fxy.id[type](value)) return value
	if(type in fxy.as) return fxy.as[type](value)
	return null
}
...
function get_type_description(value){
	...
	if(fxy.is.text(value)){
		...
		if( !(data.type in fxy.as) ) throw new Error(`${data.type} is not a valid sxy.scalar type ${Object.keys(fxy.as)}`)
	}
	return data
}
