			...
			for(let resolver in shared_resolvers){
				if(['Out','In','Push'].includes(resolver) === false){
					if(resolver in types.resolvers){
						...
						types.resolvers[resolver] = merge(shared_resolvers[resolver],types.resolvers[resolver])
					}else{
...
function get_folders(folder){
	return fxy.ls(folder).dirs.filter(dir=>dir !== '.DS_Store')
}

function get_resolvers(types,folder){
	let resolvers = require('./resolvers')
	return resolvers( merge(...(types.map(type=>type.resolvers))), folder )
}
