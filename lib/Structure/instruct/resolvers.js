...
function get_queries(folder){
	...
	function queries(typepath) {
		...
		if (fxy.exists(query_file)) {
			...
			if (typeof query === 'function') query = query(merge, fxy)
			if (Array.isArray(query)) query = merge(...query)
...
function has_mutliple_values(query,info){
	return values.filter(value=>{
			return info[value] in query
		}).length > 0
}
