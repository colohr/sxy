...
function get_types(information){
	...
	Object.defineProperty(information,'schema', {
		get(){
			...
			const resolvers = get_query_resolvers( merge(...type_resolvers), information.root_names, folder )
...
function get_type( name, directory ){
	return {
		get resolvers(){
			let resolvers = [{}]
			let type = this.type
			if('resolver' in type) resolvers.push( type.resolver || {} )
			let scalar_path = fxy.join(directory,'scalars')
			if(fxy.exists(scalar_path)) {
				let scalars = new Set( fxy.ls( scalar_path ).files('js').filter( name => !('index.js'.includes(name))) )
				let output = {}
		...
		get schema(){
			...
			let scalars = fxy.join( directory, 'scalars','index.graphql' )
			if(fxy.exists(scalars)) schemas.push( fxy.readFileSync( scalars, 'utf8') )
			return schemas.join('\n')
		},
		get type(){
			return require(fxy.join(directory,'/index.js'))
		}
	}
}

function get_query_resolvers( resolvers, {output, input, push} , folder){
	
	const dirpaths = fxy.ls(folder).dirs.filter(name=>name!=='Query').map(name=>fxy.join(folder,name))
	...
	function get_queries(typepath) {
		...
		if (fxy.exists(main_query_path)) {
			...
			if (typeof res === 'object' && res !== null) {
				...
				if ('In' in out || 'Out' in out || 'Push' in out) return out
				out.Out = res
				return out
			}
		}
