...
//exports
module.exports = function struct_language(...x){
	...
	if(typeof x[2] === "function") x[2].bind(x[0])
	const db = {
		sql,
		graph:x[0],
		language:x[2]()
	...
	return new Proxy(db,{
		get(o,name){
			...
			else if(name in o.sql){
				value = o.sql[name]
				if(typeof value === 'function') value = value.bind(o.sql)
			}
			else if(name in o.language){
				value = o.language[name]
				if(typeof value === 'function') value = value.bind(o.language)
			}
			else if(name in o.graph){
				value = o.graph[name]
				if(typeof value === 'function') value = value.bind(o.graph)
			}
			return value
		}
	})
...
function is_valid_struct({root_names,data_types},name,type){
	...
	else if(root_names && Object.keys(root_names).filter(root_name=>root_names[root_name]===name).length > 0 ) return false
